...
##now start actually cleaning and processing the data
...
#remove samples BDL - don't remove the equilibrated headspace ones BDL, but the atmospheric ones BDL?
concs$concentrationCO2.x[concs$concentrationCO2.x<concs$runDetectionLimitCO2.x]<-0.5*mean(concs$runDetectionLimitCO2.x, na.rm=TRUE)
concs$concentrationCH4.x[concs$concentrationCH4.x<concs$runDetectionLimitCH4.x]<-0.5*mean(concs$runDetectionLimitCH4.x, na.rm=TRUE)
concs$concentrationN2O.x[concs$concentrationN2O.x<concs$runDetectionLimitN2O.x]<-0.5*mean(concs$runDetectionLimitN2O.x, na.rm=TRUE)
...
concs$pCO2<-concs$CO2aq/((concs$ptBarometricPressure*1000)*(concs$HCO2_field/1000)) #1000 =  conversion H to L
#partial pressures in ppm
...
concs$pCO2_err<-(concs$CO2aq+concs$CO2aq_err)/((concs$ptBarometricPressure*1000)*(concs$HCO2_field/1000))-
  concs$CO2aq/((concs$ptBarometricPressure*1000)*(concs$HCO2_field/1000))
...
concs$pN2O<-concs$N2Oaq/((concs$ptBarometricPressure*1000)*(concs$HN2O_field/1000))

concs$pN2O_err<-(concs$N2Oaq+concs$CO2aq_err)/((concs$ptBarometricPressure*1000)*(concs$HN2O_field/1000))-
  concs$N2Oaq/((concs$ptBarometricPressure*1000)*(concs$HN2O_field/1000))
...
concs$pCH4_err<-(concs$CH4aq+concs$CO2aq_err)/((concs$ptBarometricPressure*1000)*(concs$HCH4_field/1000))-
  concs$CH4aq/((concs$ptBarometricPressure*1000)*(concs$HCH4_field/1000))
...
concs$pCH4<-concs$CH4aq/((concs$ptBarometricPressure*1000)*(concs$HCH4_field/1000))
...
concs_sum<- concs %>%
  dplyr::select(siteID, date, datetime, storageWaterTemp, fieldtemp, ptBarometricPressure,
                CO2aq, pCO2, pCO2_err, CO2aq_err, CO2_excess, CO2_pctsat, CH4aq, pCH4, pCH4_err, CH4aq_err, 
                CH4_excess, CH4_pctsat, N2Oaq, pN2O, pN2O_err, N2Oaq_err, N2O_excess,
                N2O_pctsat, genflag_binary)

concs_sumdate<- concs_sum %>%
  group_by(siteID, date) %>%
  summarise(CO2aq1 = mean(CO2aq, na.rm=TRUE),
            pCO21 = mean(pCO2, na.rm=TRUE),
...
concs_sum1<- concs %>%
  ...
  summarise(CO2aq1 = mean(CO2aq, na.rm=TRUE),
            pCO21 = mean(pCO2, na.rm=TRUE),
            pCO2_err1 = mean(pCO2_err, na.rm=TRUE),
            CO2_excess = mean(CO2_excess, na.rm=TRUE),
            CO2_pctsat = mean(CO2_pctsat, na.rm=TRUE),
            CO2aq_cv = sd(CO2aq, na.rm=TRUE)/mean(CO2aq, na.rm=TRUE),
            CH4aq1 = mean(CH4aq, na.rm=TRUE),
            pCH41 = mean(pCH4, na.rm=TRUE),
            pCH4_err1 = mean(pCH4_err, na.rm=TRUE),
            CH4_excess = mean(CH4_excess, na.rm=TRUE),
            CH4_pctsat = mean(CH4_pctsat, na.rm=TRUE),
            CH4aq_cv = sd(CH4aq, na.rm=TRUE)/mean(CH4aq, na.rm=TRUE),
            N2Oaq1 = mean(N2Oaq, na.rm=TRUE),
            pN2O1 = mean(pN2O, na.rm=TRUE),
            pN2O_err1 =mean(pN2O_err, na.rm=TRUE),
            N2O_excess = mean(N2O_excess, na.rm=TRUE),
            N2O_pctsat = mean(N2O_pctsat, na.rm=TRUE),
            N2Oaq_cv = sd(N2Oaq, na.rm=TRUE)/mean(N2Oaq, na.rm=TRUE),
            storageWaterTemp = mean(storageWaterTemp, na.rm=TRUE),
...
write.csv(concs_sum1, 'concs_sum1.csv')
